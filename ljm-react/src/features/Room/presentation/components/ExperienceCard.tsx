...
const ExperienceCard: FC<ExperienceProps> = ({ title, description, image, isSelected, onSelect }) => {
  return (
    <div
      onClick={onSelect}
      className={`glass-card rounded-xl p-3 flex h-full flex-col gap-4 group cursor-pointer transition-all duration-300 border ${
        isSelected 
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-3 px-2 pb-2">
        ...
        <button
          className={`mt-auto w-full py-2.5 rounded-full text-[10px] font-bold tracking-[0.2em] uppercase transition-all duration-300 border ${
            isSelected 
