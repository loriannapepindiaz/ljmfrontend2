...
const PersonalizationPage: FC = () => {
  // Estado para manejar la suite seleccionada
  const [selectedSuiteId, setSelectedSuiteId] = useState<number | null>(null);

  // Datos de las suites (Precios en 0 por falta de conexión a BD)
  const suites = [
    { 
      id: 0, 
  ...
  // Buscamos la suite seleccionada para el SummaryBar
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-20">
          ...
          <section className="mb-14">
            <div className="max-w-4xl">
              <h1 className="text-5xl md:text-7xl font-serif font-bold text-[#0e1a34] mb-4 leading-tight tracking-tight uppercase tracking-[0.2em]">
                Personalice su Experiencia
              </h1>
              <p className="text-xl text-[#0e1a34]/50 font-light leading-relaxed mb-12 max-w-2xl">
            ...
            <div className="flex items-center gap-4 py-8 border-t border-[#0e1a34]/10 mb-10">
              <span className="material-symbols-outlined text-[#0e1a34] text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>king_bed</span>
              <h3 className="text-xs font-bold text-[#0e1a34] uppercase tracking-[0.5em] font-serif leading-tight">
          ...
          {/* BARRA DE RESUMEN (Flotante visualmente entre secciones) */}
          <div className="my-20">
            <SummaryBar 
              selectedSuiteName={selectedSuite ? selectedSuite.title : null} 
              totalPrice={selectedSuite ? selectedSuite.price : 0} 
            />
          </div>

          {/* Sección de Experiencias (Adaptada a la segunda foto con tres objetos) */}
          <section className="mb-24">
            <div className="flex items-center gap-4 mb-10 border-b border-[#0e1a34]/10 pb-6">
              <span className="material-symbols-outlined text-[#0e1a34] text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>diamond</span>
              <h3 className="text-xs font-bold text-[#0e1a34] uppercase tracking-[0.6em] leading-tight font-serif">
                Experiencias & Actividades Exclusivas
              </h3>
            </div>
            
            {/* Contenedor Premium de Experiencias con sombras y bordes suaves */}
            <div className="bg-white rounded-[3rem] overflow-hidden shadow-[0_25px_60px_rgba(14,26,52,0.12)] border border-[#0e1a34]/5 divide-y divide-gray-100">
                <ExperienceRow 
                  title="Safari en Jet Ski: Calas Ocultas" 
                  price={0} 
                  unit="Invitado" 
                  image="https://images.unsplash.com/photo-1545652925-af5505e60d3d?auto=format&fit=crop&q=80" 
          ...
          <div className="mb-20">
            <button className="group w-full bg-[#785d32] hover:bg-[#5a4626] text-white font-bold py-9 px-12 rounded-[3rem] text-xs tracking-[0.5em] uppercase transition-all shadow-2xl flex items-center justify-center gap-4 active:scale-[0.99]">
              03 CONFIRMAR RESERVA
              <span className="material-symbols-outlined text-2xl group-hover:translate-x-3 transition-transform">
                arrow_forward
              </span>
            </button>
          </div>
